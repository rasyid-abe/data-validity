...
def query_data_source():
    with con_source.cursor() as c_source:
        c_source.execute(f"""
            ...
            WHERE
            	t.transaction_tgl BETWEEN '{yesterday}' AND '{today}'
...
def query_data_target():
    with con_target.cursor() as c_target:
        c_target.execute(f"""
            SELECT
            	M_User_id_user user,
            	COUNT(raw_mart_sales_addon_id_transaction_addon_detail) total
            FROM
            	Raw_Mart_Sales_Addon
            WHERE
            	raw_mart_sales_addon_datetime BETWEEN '{yesterday}' AND '{today}'
            GROUP BY
            	M_User_id_user
        """)
...
def history_validity(data):
    ...
    """
    ins = ['Varian', data[0], data[1], data[2], data[3], datetime.now(), yesterday, today]
