    ...
    with con_source.cursor() as c_source:
        c_source.execute(f"""
            SELECT
            	...
            	COUNT(id_transaction_detail) rows
            FROM
            	Transactions t
...
def query_data_target():
    with con_target.cursor() as c_target:
        c_target.execute(f"""
            SELECT
            	M_User_id_user user,
            	COUNT(raw_mart_sales_transaction_id_transaction_detail) total
            FROM
            	Raw_Mart_Sales_Transaction
            WHERE
            	raw_mart_sales_transaction_datetime BETWEEN '{yesterday}' AND '{today}'
            GROUP BY
            	M_User_id_user
        """)
...
def history_validity(data):
    ...
    """
    ins = ['Produk', data[0], data[1], data[2], data[3], datetime.now(), yesterday, today]
